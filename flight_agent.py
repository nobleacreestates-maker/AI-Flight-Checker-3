"""
AI Flight Search Agent - Fixed Restaurant & Itinerary Generation
"""
...
class TravelPlanningAgent:
    ...
    def create_structured_itinerary(self, destination, keywords, budget, duration_days, hotels):
        """Create comprehensive structured itinerary with AI - GUARANTEED OUTPUT"""
        hotel_info = ""
...
IMPORTANT: Respond ONLY with the JSON object. No markdown, no code blocks, no other text."""
        ...
        try:
            ...
            if "restaurants" not in itinerary_data:
                ...
                itinerary_data["restaurants"] = self._get_default_restaurants(destination)
            ...
            if "daily_itinerary" not in itinerary_data or len(itinerary_data["daily_itinerary"]) == 0:
                ...
                itinerary_data["daily_itinerary"] = self._get_default_itinerary(destination, duration_days)
            ...
            return self._get_fallback_itinerary(destination, duration_days)
        except Exception as e:
            print(f"Itinerary creation error: {e}")
            return self._get_fallback_itinerary(destination, duration_days)
    
    def _get_default_restaurants(self, destination):
        """Fallback restaurant data"""
        return {
            "breakfast": [
                {
                    "name": f"{destination} Breakfast Café",
                    "cuisine": "Local",
                    "price_per_person": 12,
                    "rating": 4.5,
    ...
    def _get_default_itinerary(self, destination, days):
        ...
        for day in range(1, days + 1):
            itinerary.append({
                ...
                "morning": {
                    ...
                    "description": f"Explore {destination} in the morning",
                    "cost": 15,
                    "duration": "2-3 hours",
    ...
    def _get_fallback_itinerary(self, destination, duration_days):
        ...
        return {
            ...
            "daily_itinerary": self._get_default_itinerary(destination, duration_days),
            "restaurants": self._get_default_restaurants(destination),
            "budget_summary": {
                "activities": 200,
                "food": 300,
                "transport": 50,
...
def create_itinerary():
    ...
    if accommodation_type in ['airbnb', 'mixed']:
        ...
        print(f"Found {len(airbnb_options)} Airbnb options")
    ...
    # Create itinerary - GUARANTEED TO WORK
